  ...
  render() {
    ...
    return (
      <Container className="todo-app">
        <Row>
          <Col md={12}>
            <h3 className="page-title">{t('Campagne de prévention')}</h3>
          </Col>
        </Row>
        <Row>
          <Col md={9} xl={10}>
          ...
          <Col md={3} xl={2}>
            <TodoInput addTodo={actions.addTodo} togglePriorityFilter={actions.togglePriorityFilter} theme={theme} />
          </Col>
        </Row>
      </Container>
    );
...
function mapStateToProps(state) {
  const todos = state.todos.priorityFilter
    ...
    : state.todos.todos;
  return { todos, theme: state.theme };
}
